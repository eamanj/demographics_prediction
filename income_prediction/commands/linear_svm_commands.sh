...
RESULTS_DIR="${TOP_LEVEL_DIR}/results/bandicoot/linear-svm"
...
do
  ...
  do
    ...
    ${SCRIPTS_DIR}/svm.py -trs $TRAIN_SIZE -tes $TEST_SIZE \
      -k linear -s minmax -lc $LABEL_COLUMN -e $EVALUATION -nj $NUM_JOBS \
...
do
  ...
  do
    ${SCRIPTS_DIR}/svm.py -trs $TRAIN_SIZE -tes $TEST_SIZE \
      -k linear -s minmax -lc $LABEL_COLUMN -e $EVALUATION -nj $NUM_JOBS -i \

...
european_data = read.csv(file = "cleaned_european_data.csv", header = TRUE, sep = ",")
setwd(dir = "~/research/gender_prediction/european_plots/")
num_data_points = nrow(european_data)

numerics = sapply(european_data, is.numeric)
for (column in names(european_data)) {
  column_data = european_data[[column]]
  if (!numerics[column]) {

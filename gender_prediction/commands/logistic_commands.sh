...
RESULTS_DIR="${TOP_LEVEL_DIR}/results/extended_indicators/logistic-regression"
...
do
  ...
  do
    ...
    do
      if [ "$GRID_SEARCH" = true ]
      then
        ${SCRIPTS_DIR}/run_scikit_logistic.py \
        ...
        # south asia with/without sb and l1/l2 losses
        ${SCRIPTS_DIR}/run_scikit_logistic.py \
          -trs $TRAIN_SIZE -tes $TEST_SIZE -sg $COMMON_FLAGS -p l1 -c 1 -lc $SA_LABEL_COLUMN \
          -o ${SOUTH_ASIAN_PREDICTIONS_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l1.c_1 \
          $SOUTH_ASIAN_INPUT_DATA \
          > ${SOUTH_ASIAN_RESULT_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l1.c_1

        ${SCRIPTS_DIR}/run_scikit_logistic.py \
          -trs $TRAIN_SIZE -tes $TEST_SIZE -sg $COMMON_FLAGS -p l1 -c 1 -lc $SA_LABEL_COLUMN -sb \
          -o ${SOUTH_ASIAN_PREDICTIONS_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l1.c_1.sb \
          $SOUTH_ASIAN_INPUT_DATA \
          > ${SOUTH_ASIAN_RESULT_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l1.c_1.sb
       
        ${SCRIPTS_DIR}/run_scikit_logistic.py \
          -trs $TRAIN_SIZE -tes $TEST_SIZE -sg $COMMON_FLAGS -p l2 -c 10 -lc $SA_LABEL_COLUMN \
          -o ${SOUTH_ASIAN_PREDICTIONS_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l2.c_10 \
          $SOUTH_ASIAN_INPUT_DATA \
          > ${SOUTH_ASIAN_RESULT_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l2.c_10
       
        ${SCRIPTS_DIR}/run_scikit_logistic.py \
          -trs $TRAIN_SIZE -tes $TEST_SIZE -sg $COMMON_FLAGS -p l2 -c 10 -lc $SA_LABEL_COLUMN -sb \
          -o ${SOUTH_ASIAN_PREDICTIONS_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l2.c_10.sb \
          $SOUTH_ASIAN_INPUT_DATA \
          > ${SOUTH_ASIAN_RESULT_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l2.c_10.sb
       
        # european without sb and l1/l2 losses
        ${SCRIPTS_DIR}/run_scikit_logistic.py \
          -trs $TRAIN_SIZE -tes $TEST_SIZE -sg $COMMON_FLAGS -p l1 -c 1 -lc $EU_LABEL_COLUMN \
          -o ${EUROPEAN_PREDICTIONS_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l1.c_1 \
          $EUROPEAN_INPUT_DATA \
          > ${EUROPEAN_RESULT_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l1.c_1
       
        ${SCRIPTS_DIR}/run_scikit_logistic.py \
          -trs $TRAIN_SIZE -tes $TEST_SIZE -sg $COMMON_FLAGS -p l2 -c 10 -lc $EU_LABEL_COLUMN \
          -o ${EUROPEAN_PREDICTIONS_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l2.c_10 \
          $EUROPEAN_INPUT_DATA \
          > ${EUROPEAN_RESULT_PREFIX}.trial_${TRIAL}.trs_${TRAIN_SIZE}.tes_${TEST_SIZE}${COMMON_SUFFIX}.p_l2.c_10
      fi
    done
  done
done

...
parser = argparse.ArgumentParser(description='Script replaces "None" strings or missing '
    'data in raw European data or South Asian data, removes some bad rows according to '
    'some filters such as threshold on min active days and min number of contacts, and '
    'writes the cleaned version to output file. The cleaned version is a subset of the '
    'input file in both rows and columns, with the exception that "None" string or '
    'missing data is replaced.')
...
parser.add_argument('-sa', '--south_asian', dest='south_asian_data', default=False,
                    action='store_true',
                    help='Whether this data is south asian or not. Each data has '
                    'its own protocol for labeling gender.')
...
def main():
  ...
  # Manually add gender and age at the end of indicators so that labels are last two
  # columns
  indicators = list()
  ...
  indicators.sort()
  indicators.append(gender_column_name)
  indicators.append(age_column_name)
  ...
  # How genders are represented in each data set
  ...
  for index, row in df.iterrows():
    ...
    features = list()
    # Add gender and age after the loop, to make sure they appear last in the list, just
    # like header
    for indicator in indicators:
      feature = row[indicator] if not data_missing(row[indicator]) else na_replacement
      # Make sure both european and south asian label gender the same in the cleaned data.
      if "gender" in indicator:
        gender = str(utils.FEMALE) if feature == female else str(utils.MALE)
